...
public class Nsfw : MewdekoModuleBase<ISearchImagesService>
{
    ...
    {
        var msg = await ctx.Channel.SendConfirmAsync($"<a:loadingstate:1138172643867111595> Trying to get a post from `{subreddit}`...");
        try
        {
            RedditPost image;
            try
            ...
            catch (Exception e)
            {
                ...
                await ctx.Channel.SendErrorAsync("Unable to fetch nsfw subreddit. Please check console or report the issue at https://discord.gg/C3yyk7ebEz.");
                return;
            }
